...
// filtered  list
  ...
  //  function filterForUT (UT_Data,SearchText_FromBtns){
  //          const filterItems= UT_Data.filter((data)=> data?.snippet?.description?.toLowerCase().includes(SearchText_FromBtns))
  //          return filterItems
  //     }
...
//  
   return( <div className="border border-blue-800 w-[100%] px-0 py-0 mt-16 h-[100%]">
    ...
    scrollbar-w-thin scrollbar-rounded-full items-center scrollbar-thumb-gray-600 scrollbar-track-gray-200  snap-x">
            ...
            onClick={()=>{ DoSearch(Btn_text.toLowerCase())} } key={Math.random()*2 }>{Btn_text}</button>) }
    </div>
...
{/* <div className={FilterItemFromState.length === 0?"border border-gray-400 h-[6rem] mt-[5rem] hidden items-center justify-center ":"border border-gray-400 h-[6rem] mt-[5rem] flex items-center justify-center"}>
